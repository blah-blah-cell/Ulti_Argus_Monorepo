...
# Kronos trainer is optional — pipeline works fine without it
try:
    from ..kronos.trainer import KronosTrainer
    _KRONOS_TRAINER_AVAILABLE = True
except ImportError:
    _KRONOS_TRAINER_AVAILABLE = False
    KronosTrainer = None  # type: ignore
...
class MnemosynePipeline:
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            # Initialize Kronos trainer if available
            if _KRONOS_TRAINER_AVAILABLE:
                kronos_model_path = getattr(
                    self.config, "kronos_model_path",
                    "/var/lib/argus_v/kronos/kronos_router.pkl"
                )
                self._kronos_trainer = KronosTrainer(
                    model_save_path=kronos_model_path
                )
                log_event(
                    logger,
    ...
    def run_training_pipeline(self, max_training_data_age_hours: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 3b: Train Kronos meta-router (secondary target)
            if self._kronos_trainer is not None:
                log_event(logger, "step_3b_kronos_training", level="info")
                try:
                    kronos_stats = self._kronos_trainer.train(samples=None)
                    pipeline_stats['kronos_training'] = kronos_stats
                    log_event(
                        logger,
                        "kronos_training_completed",
                        level="info",
                        kronos_stats=kronos_stats,
                    )
                except Exception as kronos_err:
                    # Non-fatal — IF model still works without Kronos
                    log_event(
                        logger,
                        "kronos_training_failed",
                        level="warning",
                        error=str(kronos_err),
                    )
                    pipeline_stats['kronos_training'] = {'error': str(kronos_err)}
